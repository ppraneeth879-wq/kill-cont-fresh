...
| Real Google sign-in (Bundle F) | Completed (code) — awaiting user provisioning | Open access policy (any Google account → demo org as `operator`). `repos.upsert_user` accepts `role` kwarg; `/session/login` passes `role="operator"` for firebase mode. Startup banner via `print()` reports `AUTH_BACKEND` + credentials path. SignInPage handles popup-cancel + popup-blocked + unauthorized-domain errors cleanly. New `infra/google-cloud/SETUP.md` walks through Firebase Console + Cloud Run + Firebase Hosting end-to-end. **Demo mode `AUTH_BACKEND=demo` remains the default** (offline-friendly). |
